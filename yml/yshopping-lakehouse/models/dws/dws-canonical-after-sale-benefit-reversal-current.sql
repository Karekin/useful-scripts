...
WITH expected_entitlements AS (
    ...
    SELECT reversal.*,
           SUM(reversal.amount_minor) OVER (
               PARTITION BY reversal.tenant_id, reversal.order_id, reversal.benefit_allocation_id
               ORDER BY reversal.recorded_at, reversal.event_id
               ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
           ) AS allocation_reversed_through_event,
           SUM(reversal.amount_minor) OVER (
               PARTITION BY reversal.tenant_id, reversal.order_id, reversal.benefit_application_id
               ORDER BY reversal.recorded_at, reversal.event_id
               ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
           ) AS application_reversed_through_event
    ...
    SELECT reversal.tenant_id, reversal.after_sale_id, reversal.reversal_batch_id,
           ...
           COUNT(DISTINCT IF(reversal.entitlement_id IS NOT NULL,
                             reversal.benefit_application_id, NULL)) AS entitlement_effect_count,
           COUNT(DISTINCT IF(reversal.entitlement_effect_status = 'RETAINED_PARTIAL',
                             reversal.benefit_application_id, NULL)) AS retained_entitlement_count,
           COUNT(DISTINCT IF(reversal.entitlement_effect_status = 'RETURNED',
                             reversal.benefit_application_id, NULL)) AS returned_entitlement_count,
           ...
           SUM(CASE WHEN reversal.payload_benefit_reversal_id <> reversal.benefit_reversal_id
                  ...
                  OR reversal.amount_minor <= 0
                  OR reversal.amount_minor > allocation.amount_minor
                  OR reversal.allocation_reversed_through_event > allocation.amount_minor
                  OR reversal.application_reversed_through_event > application.amount_minor
           ...
           SUM(CASE
                 WHEN reversal.entitlement_id IS NULL
                 ...
                 WHEN reversal.entitlement_id IS NOT NULL
                      ...
                      AND reversal.application_reversed_through_event < application.amount_minor
                      AND reversal.entitlement_effect_status = 'RETAINED_PARTIAL' THEN 0
                 WHEN reversal.entitlement_id IS NOT NULL
                      AND reversal.benefit_type = 'COUPON'
                      AND reversal.benefit_source_type = 'COUPON_ENTITLEMENT'
                      AND reversal.benefit_source_id = reversal.entitlement_id
                 ...
                 ELSE 1
               END) AS entitlement_effect_mismatch_count,
           MAX(reversal.recorded_at) AS benefit_reversal_recorded_at
    FROM reversal_history reversal
    ...
    SELECT funding.*,
           SUM(funding.amount_minor) OVER (
               PARTITION BY funding.tenant_id, funding.order_id, funding.benefit_funding_id
               ORDER BY funding.recorded_at, funding.funding_reversal_id
               ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
           ) AS funding_reversed_through_event,
           SUM(funding.amount_minor) OVER (
               PARTITION BY funding.tenant_id, funding.benefit_reversal_id
           ) AS reversal_funding_amount_minor
    ...
    SELECT funding.tenant_id, funding.after_sale_id, funding.reversal_batch_id,
           ...
           SUM(CASE WHEN original.benefit_funding_id IS NULL
                  ...
                  OR funding.amount_minor <= 0
                  OR funding.amount_minor > original.amount_minor
                  OR funding.funding_reversed_through_event > original.amount_minor
                  OR funding.reversal_funding_amount_minor <> funding.benefit_reversal_amount_minor
                  OR funding.currency_code <> original.currency_code
               THEN 1 ELSE 0 END) AS funding_reversal_mismatch_count
    FROM funding_history funding

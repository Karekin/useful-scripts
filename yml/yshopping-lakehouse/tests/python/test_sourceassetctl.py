...
class SourceAssetCtlTest(unittest.TestCase):
    ...
    def test_domain_routing_is_measured_but_never_credited_as_final_disposition(self):
        ...
        self.assertTrue(any("dim_derived=43/43 (100.00%)" in line for line in lines))
        self.assertTrue(any("dim_derived=0/43 (0.00%)" in line for line in lines))
        self.assertTrue(any("ads_derived=52/52 (100.00%)" in line for line in lines))
    ...
    def test_dim_derived_dispositions_cover_exact_40_dimensions_and_3_wrong_layer_sources(self):
        contract = SOURCE_ASSETS.load_dim_derived_source_dispositions()
        assets = contract["assets"]
        names = {asset["source_asset"] for asset in assets}
        self.assertEqual(43, len(names))
        self.assertEqual(40, sum(name.startswith("dim_") for name in names))
        self.assertEqual(
            {
                "ods_ai_workflow_account_ri",
                "ods_ai_workflow_app_ri",
                "ods_intelligence_event_code_level_ri",
            },
            {name for name in names if name.startswith("ods_")},
        )
        self.assertEqual([], SOURCE_ASSETS.validate_dim_derived_source_dispositions(self.inventory))

    def test_dim_derived_profiles_repair_targets_split_authority_and_protect_sensitive_data(self):
        contract = SOURCE_ASSETS.load_dim_derived_source_dispositions()
        by_name = {asset["source_asset"]: asset for asset in contract["assets"]}
        profiles = contract["semantic_profiles"]
        self.assertIn("dim_cs_ticket_source_df", " ".join(by_name["dim_coupon_template_df"]["corrections"]))
        self.assertIn("dim_scm_warhouse_df", " ".join(by_name["dim_scm_user_df"]["corrections"]))
        self.assertIn("split merchant account shop legal entity", " ".join(by_name["dim_merchant_df"]["corrections"]))
        self.assertIn("tokenized or vaulted", profiles["restricted_principal_projection"]["security"])
        self.assertIn("Passwords, salts, API keys and Redis credentials are excluded", profiles["ai_workflow_projection"]["security"])
        self.assertIn("Listing, Pricing and Inventory retain their own authority", profiles["catalog_product_version"]["authority"])

    def test_dim_derived_status_credits_definition_not_runtime_or_final(self):
        status = SOURCE_ASSETS.dim_derived_disposition_status(self.inventory)
        self.assertEqual(43, status["dim_derived_source_asset_count"])
        self.assertEqual(43, status["dim_derived_detailed_disposition_specified_count"])
        self.assertEqual(100.0, status["dim_derived_detailed_disposition_percent"])
        self.assertEqual(0, status["dim_derived_runtime_nonempty_reconciled_count"])
        self.assertEqual(0, status["dim_derived_final_disposition_verified_count"])

    def test_dim_derived_dispositions_reject_missing_duplicate_weakened_and_create_drift(self):
        contract = SOURCE_ASSETS.load_dim_derived_source_dispositions()
        missing = json.loads(json.dumps(contract))
        missing["assets"].pop()
        errors = SOURCE_ASSETS.validate_dim_derived_source_dispositions(self.inventory, missing)
        self.assertTrue(any("differ from same-name CREATE" in error for error in errors))

        duplicate = json.loads(json.dumps(contract))
        duplicate["assets"][1] = json.loads(json.dumps(duplicate["assets"][0]))
        errors = SOURCE_ASSETS.validate_dim_derived_source_dispositions(self.inventory, duplicate)
        self.assertTrue(any("duplicate source_asset" in error for error in errors))

        weakened = json.loads(json.dumps(contract))
        weakened["assets"][-1]["decision"] = "derive"
        errors = SOURCE_ASSETS.validate_dim_derived_source_dispositions(self.inventory, weakened)
        self.assertTrue(any("wrong-layer ODS asset must be rejected" in error for error in errors))

        drifted = json.loads(json.dumps(contract))
        drifted["assets"][0]["detail_line"] = 1760
        errors = SOURCE_ASSETS.validate_dim_derived_source_dispositions(self.inventory, drifted)
        self.assertTrue(any("CREATE evidence differs" in error for error in errors))

    def test_all_dim_derived_implementation_references_resolve(self):
        contract = SOURCE_ASSETS.load_dim_derived_source_dispositions()
        references = [
            reference
            for values in contract["implementation_refs"].values()
    ...
    def test_ads_derived_dispositions_cover_exact_50_products_and_2_wrong_layer_facts(self):
        contract = SOURCE_ASSETS.load_ads_derived_source_dispositions()
        names = {asset["source_asset"] for asset in contract["assets"]}
        self.assertEqual(52, len(names))
        self.assertEqual(50, sum(name.startswith("ads_") for name in names))
        self.assertEqual(
            {"dwd_com_trend_detail_ri", "dwd_platform_intelligence_trend_ri"},
            {name for name in names if name.startswith("dwd_")},
        )
        self.assertEqual([], SOURCE_ASSETS.validate_ads_derived_source_dispositions(self.inventory))

    def test_ads_derived_profiles_separate_content_transport_ai_and_business_authority(self):
        contract = SOURCE_ASSETS.load_ads_derived_source_dispositions()
        by_name = {asset["source_asset"]: asset for asset in contract["assets"]}
        profiles = contract["semantic_profiles"]
        self.assertEqual("reject", by_name["ads_risk_csr_repay_buyer_content"]["decision"])
        self.assertIn("tatget", " ".join(by_name["ads_risk_csr_ai_ticket_tatget"]["corrections"]))
        self.assertIn("ovewview", " ".join(by_name["ads_operation_com_ovewview_target"]["corrections"]))
        self.assertIn("Kafka delivery records attempt acknowledgement and retry", " ".join(profiles["monitoring_alert_delivery"]["metric_rules"]))
        self.assertIn("raw ticket content", " ".join(profiles["risk_case_content_projection"]["metric_rules"]))
        self.assertIn("Direct identifiers", profiles["customer_profile_analytics"]["security"])

    def test_ads_derived_status_credits_definition_not_runtime_or_final(self):
        status = SOURCE_ASSETS.ads_derived_disposition_status(self.inventory)
        self.assertEqual(52, status["ads_derived_source_asset_count"])
        self.assertEqual(52, status["ads_derived_detailed_disposition_specified_count"])
        self.assertEqual(100.0, status["ads_derived_detailed_disposition_percent"])
        self.assertEqual(0, status["ads_derived_runtime_nonempty_reconciled_count"])
        self.assertEqual(0, status["ads_derived_final_disposition_verified_count"])

    def test_ads_derived_dispositions_reject_missing_duplicate_weakened_and_create_drift(self):
        contract = SOURCE_ASSETS.load_ads_derived_source_dispositions()
        missing = json.loads(json.dumps(contract))
        missing["assets"].pop()
        errors = SOURCE_ASSETS.validate_ads_derived_source_dispositions(self.inventory, missing)
        self.assertTrue(any("differ from same-name CREATE" in error for error in errors))

        duplicate = json.loads(json.dumps(contract))
        duplicate["assets"][1] = json.loads(json.dumps(duplicate["assets"][0]))
        errors = SOURCE_ASSETS.validate_ads_derived_source_dispositions(self.inventory, duplicate)
        self.assertTrue(any("duplicate source_asset" in error for error in errors))

        weakened = json.loads(json.dumps(contract))
        weakened["assets"][-1]["decision"] = "derive"
        errors = SOURCE_ASSETS.validate_ads_derived_source_dispositions(self.inventory, weakened)
        self.assertTrue(any("wrong-layer DWD asset must be rejected" in error for error in errors))

        drifted = json.loads(json.dumps(contract))
        drifted["assets"][0]["detail_line"] = 4359
        errors = SOURCE_ASSETS.validate_ads_derived_source_dispositions(self.inventory, drifted)
        self.assertTrue(any("CREATE evidence differs" in error for error in errors))

    def test_all_ads_derived_implementation_references_resolve(self):
        contract = SOURCE_ASSETS.load_ads_derived_source_dispositions()
        references = [
            reference
            for values in contract["implementation_refs"].values()
    ...
    def test_dwd_dwm_derived_dispositions_cover_exact_205_same_name_creates(self):
        contract = SOURCE_ASSETS.load_dwd_dwm_derived_source_dispositions()
        assets = contract["assets"]
        names = {asset["source_asset"] for asset in assets}
        self.assertEqual(205, len(names))
        self.assertEqual(
            {"dim": 5, "dwd": 165, "dwm": 24, "ods": 11},
            dict(Counter(name.split("_", 1)[0] for name in names)),
        )
        self.assertEqual(
            {
                "wrong_layer": 16, "event_fact": 78, "current_projection": 73,
                "transport_bridge": 14, "wide_projection": 24,
            },
            dict(Counter(asset["physical_role"] for asset in assets)),
        )
        self.assertEqual([], SOURCE_ASSETS.validate_dwd_dwm_derived_source_dispositions(self.inventory))

    def test_dwd_dwm_profiles_protect_grain_money_content_ai_and_authority(self):
        contract = SOURCE_ASSETS.load_dwd_dwm_derived_source_dispositions()
        by_name = {asset["source_asset"]: asset for asset in contract["assets"]}
        profiles = contract["semantic_profiles"]
        self.assertIn("legacy Trade discount", " ".join(by_name["dwd_trade_order_discount_detail_di"]["corrections"]))
        self.assertIn("misspelled source alias", " ".join(by_name["dwd_com_audit_quilty_check_detail_df"]["corrections"]))
        self.assertIn("captured Payment", " ".join(profiles["token_marketplace_fact"]["metric_rules"]))
        self.assertIn("ISO currency and integer minor units", " ".join(profiles["live_economy_fact"]["metric_rules"]))
        self.assertIn("partial and multi-line returns", " ".join(profiles["aftersales_refund_fact"]["metric_rules"]))
        self.assertIn("Prompts, responses", profiles["ai_workflow_fact"]["security"])
        self.assertEqual("reject", by_name["ods_ticket_ticket_detail_ri"]["decision"])
        self.assertEqual("wide_projection", by_name["dwm_trade_order_detail_di"]["physical_role"])

    def test_dwd_dwm_derived_status_credits_definition_not_runtime_or_final(self):
        status = SOURCE_ASSETS.dwd_dwm_derived_disposition_status(self.inventory)
        self.assertEqual(205, status["dwd_dwm_derived_source_asset_count"])
        self.assertEqual(205, status["dwd_dwm_derived_detailed_disposition_specified_count"])
        self.assertEqual(100.0, status["dwd_dwm_derived_detailed_disposition_percent"])
        self.assertEqual(0, status["dwd_dwm_derived_runtime_nonempty_reconciled_count"])
        self.assertEqual(0, status["dwd_dwm_derived_final_disposition_verified_count"])

    def test_dwd_dwm_dispositions_reject_missing_duplicate_wrong_role_and_create_drift(self):
        contract = SOURCE_ASSETS.load_dwd_dwm_derived_source_dispositions()
        missing = json.loads(json.dumps(contract))
        missing["assets"].pop()
        errors = SOURCE_ASSETS.validate_dwd_dwm_derived_source_dispositions(self.inventory, missing)
        self.assertTrue(any("differ from same-name CREATE" in error for error in errors))

        duplicate = json.loads(json.dumps(contract))
        duplicate["assets"][1] = json.loads(json.dumps(duplicate["assets"][0]))
        errors = SOURCE_ASSETS.validate_dwd_dwm_derived_source_dispositions(self.inventory, duplicate)
        self.assertTrue(any("duplicate source_asset" in error for error in errors))

        wrong_role = json.loads(json.dumps(contract))
        wrong_role["assets"][20]["physical_role"] = "wide_projection"
        errors = SOURCE_ASSETS.validate_dwd_dwm_derived_source_dispositions(self.inventory, wrong_role)
        self.assertTrue(any("invalid physical role" in error for error in errors))

        drifted = json.loads(json.dumps(contract))
        drifted["assets"][0]["detail_line"] = 15100
        errors = SOURCE_ASSETS.validate_dwd_dwm_derived_source_dispositions(self.inventory, drifted)
        self.assertTrue(any("CREATE evidence differs" in error for error in errors))
